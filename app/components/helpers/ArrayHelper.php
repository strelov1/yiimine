<?php
/**
 * Класс ArrayHelper добавляет ряд функций для работы с массивами
 */
class ArrayHelper {
    ...
    public static function get($array, $key, $default = null) {
        if (!is_array($array) and !$array instanceof ArrayAccess) {
            throw new CException('First parameter must be an array or ArrayAccess object.');
        }
    ...
    public static function assoc_to_keyval($assoc, $key_field, $val_field) {
        if (!is_array($assoc) or $assoc instanceof \Iterator) {
            throw new CException('The first parameter must be an array.');
        }
    ...
    public static function to_assoc($arr) {
        if (($count = count($arr)) % 2 > 0) {
            throw new CException('Number of values in to_assoc must be even.');
        }
        $keys = $vals = array();
    ...
    public static function is_assoc($arr) {
        if (!is_array($arr)) {
            throw new CException('The parameter must be an array.');
        }
    ...
    public static function insert(array &$original, $value, $pos) {
        if (count($original) < abs($pos)) {
            Yii::t('notice', 'Position larger than number of elements in array in which to insert.');
            return false;
        }
    ...
    public static function insert_before_key(array &$original, $value, $key) {
        ...
        if ($pos === false) {
            Yii::t('notice', 'Unknown key before which to insert the new value into the array.');
            return false;
        }
    ...
    public static function insert_after_key(array &$original, $value, $key) {
        $pos = array_search($key, array_keys($original));
        if ($pos === false) {
            Yii::t('notice', 'Unknown key after which to insert the new value into the array.');
            return false;
        }
    ...
    public static function insert_after_value(array &$original, $value, $search) {
        ...
        if ($key === false) {
            Yii::t('notice', 'Unknown value after which to insert the new value into the array.');
            return false;
        }
    ...
    public static function sort($array, $key, $order = 'asc', $sort_flags = SORT_REGULAR) {
        if (!is_array($array)) {
            throw new CException('Arr::sort() - $array must be an array.');
        }
        ...
        switch ($order) {
            ...
            default:
                throw new CException('Arr::sort() - $order must be asc or desc.');
                break;
        }
    ...
    public static function replace_key($source, $replace, $new_key = null) {
        ...
        if (!is_array($source) or !is_array($replace)) {
            throw new CException('Arr::replace_key() - $source must an array. $replace must be an array or string.');
        }
    ...
    public static function merge() {
        ...
        if (!is_array($array)) {
            throw new CException('Arr::merge() - all arguments must be arrays.');
        }
