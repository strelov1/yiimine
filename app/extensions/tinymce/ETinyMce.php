...
class ETinyMce extends CInputWidget
{
   ...
   *
    ...
    * @var array
    */
   private $switchLabels = array(0=>'Текстовый режим', 1=>'HTML режим');
   ...
   /**
    * Use elfinder (file manager)
    *
    * @var boolean
    */
   private $useElFinder = true;
    ...
    * @var array
    */
   private $validEditorTemplates = array('simple', 'full', 'custom');
   ...
   /**
    * Will we use file manager
    *
    * @param boolean $value
    */
   public function setUseElFinder($value)
   {
      if (!is_bool($value))
         throw new CException(Yii::t('ETinyMCE', 'useSwitch must be boolean'));
      $this->useElFinder = $value;
   }
    ...
    * @return boolean
    */
   public function getUseElFinder()
   {
      return $this->useElFinder;
   }
   ...
   protected function makeFullEditor($url='')
   {
      ...
      $options['theme_advanced_buttons1'] = "save,|,{$sc}formatselect,fontsizeselect,|,justifyleft,justifycenter,justifyright,justifyfull,|,forecolor,backcolor,|,bold,italic,underline,strikethrough,|,sub,sup,blockquote";
	  $options['theme_advanced_buttons2'] = "cut,copy,paste,pastetext,pasteword,|,search,replace,|,undo,redo,|,removeformat,cleanup,|,bullist,numlist,|,outdent,indent,|,charmap,emotions,media,image,|,link,unlink,|,visualaid,visualchars,|,code,";
	  $options['theme_advanced_buttons3'] = "{$c2}tablecontrols,";
	  $options['theme_advanced_toolbar_location'] = "'top'";
	  $options['theme_advanced_toolbar_align'] = "'left'";
   ...
   protected function makeCompressor()
   {
	...
	protected function makeOptions($url='')
	{
      ...
      $options['fullscreen_new_window'] = false;
      ...
      if ($this->useElFinder) {
          $options['file_browser_callback'] = 'js:function(field_name, url, type, win) {
            ...
            if($("#finder").length == 0) {
                $("body").append($("<div/>").attr("id", "finder"));
                // If you want to use with module
                //$("#finder").elfinder({ zIndex: 9999999, places: "Избранное", url : "'.Yii::app()->urlManager->createUrl('extensions/modules/elfinder/controllers/').'",
                // If you want to use without module direct /assets/elfinder/ (or its better to use elfinder like plugin in tiny_mce/plugins/elfinder ?)
                $("#finder").elfinder({ zIndex: 9999999, places: "Избранное", url : "'.Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'assets').'/elfinder/connectors/connector.php?url='.Yii::app()->baseUrl.'/uploads/'.'",
                    lang : "ru",
                    dialog : { width : 900, modal : true, title : "Файловый менеджер", zIndex: 7777777 },
      ...
      if ($this->editorTemplate !== '') {
         switch ($this->editorTemplate) {
            ...
            case 'full':
               ...
               break;
           case 'custom':
               $template = $this->makeCustomEditor($url);
               break;
         }
...
EOP;
      ...
      if ($this->useElFinder) {
         ...
         $cs->registerScriptFile($assets.'/elfinder/js/i18n/elfinder.ru.js');
...
EOP;
   ...
   /**
    * A custom editor (example modified to make it more logic)
    *
    * @link http://tinymce.moxiecode.com/examples/full.php
    *
    * @param string $url the base URL for tinymce in assets
    * @return array
    */
   protected function makeCustomEditor($url='')
   {
      $options = array();
      ...
      $options['theme_advanced_toolbar_align'] = 'left';
      $options['theme_advanced_path_location'] = 'bottom';
	  $options['theme_advanced_buttons1'] = "{$sc}bold,italic,underline,strikethrough,|,pastetext,pasteword,|,undo,redo,|,code,fullscreen,|,justifyleft,justifycenter,justifyright,justifyfull,|,bullist,numlist,|,outdent,indent,|,blockquote,|,media,image,|,link,unlink";
      ...
      $options['theme_advanced_resize_horizontal'] = true;
      $options['theme_advanced_resizing'] = true;
      $options['spellchecker_languages'] = '+Russian=ru,English=en';
      if ($url !== '') {
         $options['spellchecker_rpc_url'] = $url . '/tiny_mce/plugins/spellchecker/rpc.php';
      }
